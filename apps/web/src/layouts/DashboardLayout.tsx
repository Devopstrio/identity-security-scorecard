...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    ...
    { name: 'Global Scorecard', path: '/scorecard', icon: Trophy },
    { name: 'Risk Heatmap', path: '/risk', icon: ShieldAlert },
    { name: 'Maturity Forecast', path: '/maturity', icon: Target },
    { name: 'BU Benchmarking', path: '/benchmarks', icon: BarChart3 },
    { name: 'Remediation Hub', path: '/remediation', icon: Zap },
    { name: 'Compliance Portal', path: '/compliance', icon: FileText },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Scorecard</span>}
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            <div className="w-8 h-8 rounded-full bg-rose-600/20 flex items-center justify-center text-rose-400 font-bold">
              RA
            </div>
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Risk Advisor</p>
                <p className="text-xs text-slate-500 truncate">Global Metrics</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search scorecards, metrics, BUs..." 
